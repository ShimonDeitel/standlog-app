...
struct ContentView: View {
    ...
    @State private var editingEntry: SegmentEntry?
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Standing Desk Log")
            .toolbar {
    ...
    @ViewBuilder
    private func entryRow(_ entry: SegmentEntry) -> some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                Text(entry.tag.isEmpty ? "Segment" : entry.tag)
...
struct EntryEditorView: View {
    ...
    let existing: SegmentEntry?
    let onSave: (SegmentEntry) -> Void

    init(entry: SegmentEntry?, onSave: @escaping (SegmentEntry) -> Void) {
        self.existing = entry
        self.onSave = onSave
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(existing == nil ? "Add Segment" : "Edit Segment")
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry = existing ?? SegmentEntry()
                        entry.tag = tag
                        entry.note = note
