import XCTest
@testable import StandingDeskLog

@MainActor
final class StandingDeskLogTests: XCTestCase {
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(SegmentEntry(note: "test"))
    ...
    func testCanAddBlockedAtLimitWhenNotPro() {
        let store = Store()
        store.entries = (0..<Store.freeTierLimit).map { _ in SegmentEntry() }
        XCTAssertFalse(store.canAdd(isPro: false))
    }

    func testCanAddAlwaysAllowedWhenPro() {
        let store = Store()
        store.entries = (0..<(Store.freeTierLimit + 5)).map { _ in SegmentEntry() }
        XCTAssertTrue(store.canAdd(isPro: true))
    }

    func testDeleteEntryRemovesIt() {
        let store = Store()
        let entry = SegmentEntry(note: "to delete")
        store.add(entry)
        store.delete(entry)
    ...
    func testUpdateEntryPersistsChange() {
        let store = Store()
        var entry = SegmentEntry(note: "original")
        store.add(entry)
        entry.note = "updated"
        store.update(entry)
